...
multi-line region state, line caches, external lexer loading, and projection
from Chroma token types to `term.Style`. These responsibilities form one
presentation concern and stay together behind the existing `Highlighter` and
`Span` API. `internal/core` must not import highlighting or other presentation
packages.
